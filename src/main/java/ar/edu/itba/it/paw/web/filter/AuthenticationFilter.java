...
public class AuthenticationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (req.getRequestURL().toString().contains("index")){
			chain.doFilter(request, response);
			return;
		}
		
		if (req.getRequestURL().toString().contains("products")){
			chain.doFilter(request, response);
			return;
		}
		
		if (!req.getRequestURL().toString().contains("login") && req.getSession().getAttribute("userId") == null) {
		//	resp.sendRedirect(req.getContextPath() + "/bin/user/login"); // ESTA ES LA QUE VA. POR AHORA CAMBIADO
			resp.sendRedirect(req.getContextPath() + "/bin/index/list");
